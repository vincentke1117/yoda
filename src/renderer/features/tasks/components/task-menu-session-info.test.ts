import { beforeEach, describe, expect, it, vi } from 'vitest';
import type { Conversation } from '@shared/conversations';
import { buildTaskMenuSessionFields, resolveTaskMenuSessionFields } from './task-menu-session-info';

const mocks = vi.hoisted(() => ({
  getConversationSessionInfo: vi.fn(),
  getClaudeSessionContext: vi.fn(),
  getCodexSessionContext: vi.fn(),
}));

vi.mock('@renderer/lib/ipc', () => ({
  rpc: {
    conversations: {
      getConversationSessionInfo: mocks.getConversationSessionInfo,
      getClaudeSessionContext: mocks.getClaudeSessionContext,
      getCodexSessionContext: mocks.getCodexSessionContext,
    },
  },
}));

describe('resolveTaskMenuSessionFields', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getClaudeSessionContext.mockResolvedValue(null);
    mocks.getCodexSessionContext.mockResolvedValue(null);
  });

  it('uses the resolved session title with the resolved session id', async () => {
    mocks.getConversationSessionInfo.mockResolvedValue({
      sessionId: 'actual-codex-thread',
      sessionTitle: 'Actual Codex title',
      resumeCommand: 'codex resume actual-codex-thread',
      running: true,
      tmuxEnabled: true,
      process: {
        pid: 123,
        status: 'busy',
        updatedAt: '2026-06-11T00:00:01.000Z',
      },
    });
    mocks.getCodexSessionContext.mockResolvedValue({
      rolloutPath: '/repo/.codex/rollout.jsonl',
    });

    const fields = await resolveTaskMenuSessionFields(makeConversation(), '/repo');

    expect(mocks.getConversationSessionInfo).toHaveBeenCalledWith(
      'project-1',
      'task-1',
      'conversation-1',
      '/repo'
    );
    expect(fields).toEqual({
      runtimeId: 'codex',
      sessionId: 'actual-codex-thread',
      sessionTitle: 'Actual Codex title',
      sessionTitleSource: 'runtime',
      runtimeName: 'Codex',
      workingDirectory: '/repo',
      contentSourcePath: '/repo/.codex/rollout.jsonl',
      resumeCommand: 'codex resume actual-codex-thread',
      running: true,
      tmuxEnabled: true,
      process: {
        pid: 123,
        status: 'busy',
        updatedAt: '2026-06-11T00:00:01.000Z',
      },
    });
    expect(mocks.getCodexSessionContext).toHaveBeenCalledWith(
      '/repo',
      'actual-codex-thread',
      'Actual Codex title'
    );
  });

  it('falls back to the conversation title when the resolved info omits a title', async () => {
    mocks.getConversationSessionInfo.mockResolvedValue({
      sessionId: 'actual-codex-thread',
    });

    const fields = await resolveTaskMenuSessionFields(makeConversation(), '/repo');

    expect(fields.sessionId).toBe('actual-codex-thread');
    expect(fields.sessionTitle).toBe('Yoda conversation title');
    expect(fields.sessionTitleSource).toBe('yoda');
  });

  it('includes local working directory metadata in fallback fields', () => {
    expect(buildTaskMenuSessionFields(makeConversation(), '/repo')).toMatchObject({
      runtimeId: 'codex',
      sessionId: 'conversation-1',
      sessionTitle: 'Yoda conversation title',
      sessionTitleSource: 'yoda',
      runtimeName: 'Codex',
      workingDirectory: '/repo',
    });
  });
});

function makeConversation(): Conversation {
  return {
    id: 'conversation-1',
    projectId: 'project-1',
    taskId: 'task-1',
    runtimeId: 'codex',
    title: 'Yoda conversation title',
    lastInteractedAt: '2026-06-04T06:45:36.000Z',
    isInitialConversation: true,
  };
}
